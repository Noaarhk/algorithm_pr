...
잘 사용된 경우 1
잘못사용된 경우
    다른 괄호가 나온 경우 해당 인덱스 * -1 >> stack에 열린괄호가 들어있고 pop했을 때의 괄호와 현재 인덱스의 괄호가 짝을 이루어야 한다.
    닫히지 않은 경우 마지막 인덱스 >> for 문이 끝났을 때 stack 에 열린 괄호가 남아있다.
...
def solution(s):
    ...
    for i in range(len(s)):
        open_bracket = table.values()
        close_bracket = table.keys()
        if s[i] in open_bracket:  # 열린괄호인 경우
            stack.append(s[i])
        elif s[i] in close_bracket:  # 닫힌 괄호인 경우
            if stack:
                if table[s[i]] == stack.pop():
                    answer = 1
                else:
                    return - i
            else:
