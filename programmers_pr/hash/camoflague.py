...
def solution(clothes):
    ...
    # nums_categories = list(map(lambda x: x + 1,list(cnt_categories.values())))
...
if __name__ == '__main__':
    ...
    print(solution(c))
    # cnt_categories = collections.Counter(list(map(lambda x: x[1], c)))
    # multiple = reduce(lambda x, y: (x + 1) * (y + 1), [3,4,5])
    # print(multiple)
